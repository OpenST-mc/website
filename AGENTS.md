...
- **Vercel**: `api/share.js` 社交分享卡重定向（`/share/:subid` → `/api/share?id=`，见 `vercel.json`）；`api/download.js` 下载计数 + 302 跳转、`api/stats.js` 查询下载量——两者依赖 **Upstash Redis**（Vercel 已下线 KV，需在 Vercel 后台 → Storage → Upstash 创建免费 Redis 库，自动注入 `UPSTASH_REDIS_REST_URL` / `UPSTASH_REDIS_REST_TOKEN`；未配置时降级为「仅跳转不计数」/「返回 count: null」，前端自动隐藏计数显示）。
